package com.kh.model.dto;

import java.sql.Date;

public class BoardDto {
	private int boardNo;
	private String title;
	private String content;
	private String password;
	private Date writenDate;
	public BoardDto() {
		super();
	}
	public BoardDto(String title, String content, String password) {
		super();
		this.title = title;
		this.content = content;
		this.password = password;
	}
	
	
	@Override
	public String toString() {
		return "BoardDto [boardNo=" + boardNo + ", title=" + title + ", content=" + content + ", password=" + password
				+ ", writenDate=" + writenDate + "]";
	}
	public BoardDto(int boardNo, String title, String content, String password, Date writenDate) {
		super();
		this.boardNo = boardNo;
		this.title = title;
		this.content = content;
		this.password = password;
		this.writenDate = writenDate;
	}
	public int getBoardNo() {
		return boardNo;
	}
	public void setBoardNo(int boardNo) {
		this.boardNo = boardNo;
	}
	public String getTitle() {
		return title;
	}
	public void setTitle(String title) {
		this.title = title;
	}
	public String getContent() {
		return content;
	}
	public void setContent(String content) {
		this.content = content;
	}
	public String getPassword() {
		return password;
	}
	public void setPassword(String password) {
		this.password = password;
	}
	public Date getWritenDate() {
		return writenDate;
	}
	public void setWritenDate(Date writenDate) {
		this.writenDate = writenDate;
	}
	
	
	
}
